...
def build_canonical_license_bytes() -> bytes:
    ...
    # Генерируем канонический русский текст прямо здесь, чтобы финальный repair MSI
    # не зависел от того, в каком состоянии промежуточный License.rtf оставили WiX
    # или preprocess.py на предыдущих шагах сборки.
    paragraphs = [
        (
        ...
        (
            ...
            f"{company_name}. Информационная страница: https://nanodesk.ru."
        ),
        (
            ...
            "устройствам, обработку информации на них и применение средств удалённого администрирования."
        ),
        (
            ...
            "«О персональных данных», Федеральный закон № 149-ФЗ "
            "«Об информации, информационных технологиях и о защите информации», а также иных "
            "обязательных требований по защите информации, коммерческой тайны и служебных данных."
        ),
        (
            "Если в рамках работы программы осуществляется обработка персональных данных, пользователь "
            "обязан самостоятельно определить правовые основания обработки, состав обрабатываемых данных, "
            "сроки хранения, круг допущенных лиц и необходимые организационные и технические меры защиты."
        ),
        (
            "Программа предоставляется по принципу «как есть», если иное прямо не установлено отдельным "
            "договором. Пользователь принимает на себя ответственность за законность сценария применения, "
            "корректность настроек безопасности и контроль доступа к выданной сборке."
        ),
        legal_notice,
    ]
